...
AddEventHandler("is:updateJob", function(jobName, rank)
	userJob = jobName
	userRank = rank
	if (userJob == "pompiste") and not(active) then
		active = true
		RunPompisteThread()
	else
		active = false
			...
			TriggerEvent("izone:getResultFromPlayerInAnyJobZone", userJob, function(result)
				if result ~= nil then
					...
					if result.trailer then
						if isInService and IsPedInAnyVehicle(GetPlayerPed(-1), 0) then
							if currentVeh ~= nil then
...
function SpawnTrailer(result)
	local points = result.spawnPoints
	local carHash = result.trailerHash

	local car = tonumber(carHash)
	...
	local pompisteTrailer = nil
	for i = 1, #points do
		RequestCollisionAtCoord(points[i].x, points[i].y, points[i].z)
		closestVeh = GetClosestVehicle(x, y, z, 2.0, 0, 70)
		if closestVeh ~= nil then
			pompisteTrailer = CreateVehicle(car, points[i].x, points[i].y, points[i].z, points[i].heading, true, false)
			break
		end
	end
	if pompisteTrailer == nil then
		pompisteTrailer = CreateVehicle(car, playerCoords, 90.0, true, false)
	end

	currentTrailer = pompisteTrailer
	timeTrailer = GetGameTimer()

	SetVehicleOnGroundProperly(pompisteTrailer)
	SetVehicleHasBeenOwnedByPlayer(pompisteTrailer,true)
	SetVehicleDirtLevel(pompisteTrailer, 0)
	if result.livery then
		SetVehicleLivery(pompisteTrailer, result.livery)
	end
	local netid = NetworkGetNetworkIdFromEntity(pompisteTrailer)
	SetNetworkIdCanMigrate(netid, true)
	-- NetworkRegisterEntityAsNetworked(VehToNet(medicVeh)) NotWorking in this manifest ? 
	local pompisteVeh = GetVehiclePedIsIn(GetPlayerPed(-1), false)
	if not(DoesEntityExist(pompisteVeh)) then
		pompisteVeh = GetVehiclePedIsIn(GetPlayerPed(-1), true)
	end
	AttachVehicleToTrailer(pompisteVeh, pompisteTrailer, 10.0)
	SetEntityAsMissionEntity(pompisteTrailer, true, true)
	local plateText = GetVehicleNumberPlateText(pompisteVeh)

	SetVehicleNumberPlateText(pompisteTrailer, plateText)

	TriggerServerEvent("iPompiste:spawnTrailer", plateText)
end
