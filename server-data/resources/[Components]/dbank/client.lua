    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          Citizen.CreateThread(function()
            while true do
              Wait(0)
              if (IsNearBank()) then
                drawTxt("Appuyer sur ~INPUT_CONTEXT~ pour accéder à la banque.")
              elseif (IsNearATM()) then
                drawTxt("Appuyer sur ~INPUT_CONTEXT~ pour accéder au distributeur.")
              end
            end
          end)
        end
        ...
        if IsControlJustPressed(1, 38)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('pNotify:notifyFromServer', "Tu ne peux pas accèder au guicher depuis ta voiture", "error", "topCenter", true, 5000)
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    ...
    TriggerEvent('pNotify:notifyFromServer', "Tu n'es pas à la banque.'", "error", "topCenter", true, 5000)
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent('pNotify:notifyFromServer', "Tu n'es ni à la banque, ni à proximité d'un distributeur.", "error", "topCenter", true, 5000)
  end
end)
...
--     else
--       TriggerEvent('pNotify:notifyFromServer', "Le citoyen n'a pas pu etre trouvé.'", "error", "topCenter", true, 5000)
--     end
--   else
--     TriggerEvent('pNotify:notifyFromServer', "Tu ne peux pas donner de l'argent au citoyen, tu n'es pas assez proche de lui.", "error", "topCenter", true, 5000)
--   end
-- end)
